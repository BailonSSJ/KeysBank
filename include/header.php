...
<div class="close-session">
    <?php
        if ($_SESSION['user']['perfil'] !== 'INVITED') {    //Si el usuario ya está logeado se carga el botón de logout
            echo "<form action='".(preg_match('/\/pages\//', $_SERVER['PHP_SELF']) ? '../index.php' : 'index.php')."' method='post'>";
                echo "Bienvenid@ ".$_SESSION['user']['nick'].". ";
                echo "<input type='submit' name='exit' value='Salir' class=''>";
            echo "</form>";
        }
    ?>
</div>